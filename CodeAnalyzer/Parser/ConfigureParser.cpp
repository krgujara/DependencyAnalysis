...
{ try
  { pToker = new Toker;
	...
	pParser->addRule(pEndScope);    // configure to detect and act on function definitions these will stop further rule checking by returning false
	...
	pTypedefdefination = new TypeDefinition();
	pHandleTypedef = new HandleTypeDefinition(pRepo);
	pTypedefdefination->addAction(pHandleTypedef);
	pParser->addRule(pTypedefdefination);
	pAliasDefination = new AliasDefinition;
	pHandleAliasDefination = new HandleAliasDefinition(pRepo);
	pAliasDefination->addAction(pHandleAliasDefination);
	pParser->addRule(pAliasDefination);
