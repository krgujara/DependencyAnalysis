...
// rule to detect typedef statements
  ...
  class HandleTypeDefinition : public IAction
  {
	  Repository* p_Repos;

  public:
	  HandleTypeDefinition(Repository* pRepos)
	  {
		  p_Repos = pRepos;
	  }
	  void doAction(const Scanner::ITokCollection* pTc) override
	  {
		  ...
		  //create a new ASTNode representing the alias
		  ASTNode* pElem = new ASTNode;
		  pElem->type_ = "typedef";
		  pElem->name_ = name;
		  pElem->package_ = p_Repos->package();
  ...
  public:
	  bool doTest(const Scanner::ITokCollection* pTc) override
	  {
		  ...
		  if (tc[tc.length() - 1] == ";")
		  {
			  ...
			  if (len < tc.length())
			  {
				  //check if it is a namespace then continue 
