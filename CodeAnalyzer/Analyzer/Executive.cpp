...
bool CodeAnalysisExecutive::ProcessCommandLine(int argc, char* argv[])
{
  ...
  try {
	...
	xmlPath_ = argv[argc-1];
	//std::cout << "\nXml file path:--- " << xmlPath_;
	size_t found = (xmlPath_).find(".xml");
	if (found != std::string::npos) {
		std::cout << "XML File Path found";
		std::cout << "Saving Dependency Analysis(XML Format) result in "<< xmlPath_;
	}
	else {
		patterns_.push_back(xmlPath_);
		std::cout << "\nXML Path not provided; \nDefault Path for storing dependency Analysis results : ../XML Data/data.xml";
		xmlPath_ = "../XML Data/data.xml";
	}
...
int main(int argc, char* argv[])
{
  ...
  try {
   ...
   // typeTable.createTypeTable();
	...
	for (auto item : files)
	{
		if (contains(item.first, "*.cpp")||contains(item.first,"*.h"))
		{
			for (auto file : item.second) {
