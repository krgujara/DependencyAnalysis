...
=================
		...
		Vertex<V, E> findVertexByName1(V name);
		size_t size();  //returns the size of each edge set for each vertex of graph
		void dfs(Vertex<V, E>& v, F f);  //traverse graph
        void Tarjan();             //tarjan algo to find out the strongly connected components
	    void strongComponent(Vertex v);   //utility function
...
namespace GraphLib
{
	...
	template<typename V, typename E>
	Vertex<V, E>::Vertex(V v, size_t id) : _v(v), _id(id), _mark(false), index(-1), lowlink(-1), onStack(false) {}

	//----< construct instance - creates id sequentially >-------
	template<typename V, typename E>
	Vertex<V, E>::Vertex(V v) : _v(v), _id(count++), _mark(false), index(-1), lowlink(-1), onStack(false) {}
	...
	template<typename V, typename E>
	Vertex<V, E> Graph<V, E>::findVertexByName1(V name)
	{
		Vertex<V, E> ver("dummy", -1);
